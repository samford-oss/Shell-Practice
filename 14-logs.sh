...
then
    echo -e "$R ERROR:: Please RUN the Script with ROOT USER Access $N" &>>$LOG_FILE
    exit 1 
...
then
    ...
    echo -e "mysql  is $Y already installed....Nothing to do $N" &>>$LOG_FILE
fi
...
then
    ...
    echo -e "python3 is $Y already installed....Nothing to do $N" &>>$LOG_FILE
fi
...
then
    ...
    echo -e "nginx is $Y already installed....Nothing to do $N" &>>$LOG_FILE
fi